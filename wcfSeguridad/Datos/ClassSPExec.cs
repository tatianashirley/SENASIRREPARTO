...
namespace SQLSPExecuter {

    public class ClassSPExec {
        ...
        #region "Métodos privados"
            ...
            private string AplicaFormatoMsjError(string sMensaje) {
                ...
                try {
                    ...
                    sMensajeFormateado += "El mensaje devuelto por el servidor no tiene el formato esperado\n" + sMensaje + sMensajeFormateado; 
                }
        ...
        #region "Métodos públicos"
            ...
            public ClassSPExec(Int64 iIdConexion, string sNombreProcedimiento, string sOperacion = "") {
            ...
            public Boolean AsignarValorParametro(string NombreParametro, object Valor) {
                try {
                    if (NombreParametro == "@s_cOperacion") {
                        w_sOperacion = (string)Valor;
                    }
                    if (NombreParametro == "@s_iIdConexion") {
                        w_iIdConexion = Int64.Parse(Valor.ToString());
                    }
    
                    w_ComandoSQL.Parameters[NombreParametro].Value = Valor;
                    return (true);
                } catch (Exception ex) {
                    w_PilaMsjError.Push("#DBMS#" + ex.Message);
                }
                ...
                try {
                    foreach (System.Data.SqlClient.SqlParameter prm in w_ComandoSQL.Parameters) {
                        if (prm.Direction.ToString().ToLower() != "returnvalue") {
                            ...
                            } else if (prm.SqlDbType.ToString().ToLower() == "bigint" || prm.SqlDbType.ToString().ToLower() == "int" || prm.SqlDbType.ToString().ToLower() == "smallint" || prm.SqlDbType.ToString().ToLower() == "tinyint" || prm.SqlDbType.ToString().ToLower() == "float" ) {
                                if (!Convert.IsDBNull(prm.Value) && prm.Value != null) { 
                                    Int64 TmpNum;
                                    if (Int64.TryParse(prm.Value.ToString(), out TmpNum)) { 
                                        if (w_RemplazarCeroPorDBNull && TmpNum == 0) {
                                            prm.Value = DBNull.Value;
                                        }
                                    } else {
                                        prm.Value = DBNull.Value;
                            ...
                            } else if (prm.SqlDbType.ToString().ToLower() == "money" || prm.SqlDbType.ToString().ToLower() == "decimal") {
                                if (!Convert.IsDBNull(prm.Value) && prm.Value != null) { 
                                    Decimal TmpNumDec;
                                    if (Decimal.TryParse(prm.Value.ToString(), out TmpNumDec)) { 
                                        if (w_RemplazarCeroPorDBNull && TmpNumDec == 0) {
                                            prm.Value = DBNull.Value;
                                        }
                                    } else {
                                        prm.Value = DBNull.Value;
                            ...
                            } else if (prm.SqlDbType.ToString().ToLower() == "float") {
                                if (!Convert.IsDBNull(prm.Value) && prm.Value != null) { 
                                    Double TmpNumFloat;
                                    if (Double.TryParse(prm.Value.ToString(), out TmpNumFloat)) { 
                                        if (w_RemplazarCeroPorDBNull && TmpNumFloat == 0) {
                                            prm.Value = DBNull.Value;
                                        }
                                    } else {
                                        prm.Value = DBNull.Value;
                    ...
                    foreach (System.Data.SqlClient.SqlError SQLErr in XcpSQL.Errors) {
                        w_PilaMsjError.Push("#DBMS#" + SQLErr.Message);
                    }
                    try { 
                        w_PilaMsjError.Push("#STOREDPROC#" + w_ComandoSQL.Parameters["@o_sMensajeError"].Value.ToString());
                    } catch {
                        w_PilaMsjError.Push("#STOREDPROC# ???");
                    }
                }
                w_bEstadoOK = false;
                return (false);
