...
public partial class EnvioAPS_wfrmControlDeEnvios : System.Web.UI.Page
{
    ...
    protected void gvEnvios_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            {
                ImageButton imgCERRAR = (ImageButton)e.Row.FindControl("imgCERRAR");
                imgCERRAR.ImageUrl = "~/Imagenes/24CD_Disc_1_Certificate.gif";
                imgCERRAR.CommandName = "NOCERRAR";
                imgCERRAR.ToolTip = "Este Envio todavía se está construyendo..., no puede CERRARSE!";
    ...
    protected void gvEnvios_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "CERRAR")
        {
            ...
            Label lblNumeroEnvio = (Label)gvEnvios.Rows[index].FindControl("lblNumeroEnvio"); //TemplateField Label ID="lblNumeroEnvio"

            gvEnvios.SelectedIndex = index;

            GridViewRow row = gvEnvios.SelectedRow;
            string MessageLabel = "You selected " + row.Cells[3].Text.ToString() + "."; //DataField="Gestion"

            ImageButton imgCERRAR = (ImageButton)gvEnvios.Rows[index].FindControl("imgCERRAR");

            if (!imgCERRAR.Enabled)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Message", "alert('Este Envio debe GENERAR sus MEDIOS antes de CERRARSE !!');", true);
            }
            else
            {
                objContrlEnvios.iIdConexion = IdConexion;
                objContrlEnvios.sNumeroEnvio = lblNumeroEnvio.Text;
                if (!objContrlEnvios.CierreEnvioAPS())
                {
                    //Error
                    string DetalleError = objContrlEnvios.sMensajeError;
                    string Error = "Error al realizar la operación";
                ...
                {
                    Response.Redirect("wfrmControlDeEnvios.aspx");
                }
            }
        }        
    ...
    {
        Session["fFechaCorte"] = txtFechaCorte.Text;
    ...
    protected void gvEnvioDeMedios_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            int NumeroCite = Convert.ToInt32
                (DataBinder.Eval(e.Row.DataItem, "NumeroCite"));
